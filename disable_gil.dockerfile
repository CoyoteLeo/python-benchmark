...
RUN apt-get update && apt-get install -y --no-install-recommends libuv1 zlib1g libffi-dev python-dev-is-python3
RUN apt-get install -y --no-install-recommends build-essential curl \
    && curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y \
    && pip install fastapi uvicorn gunicorn socketify --no-binary :all \
